...
describe("Injected Reward Pool - 50x stakers test", () => {
  ...
  test("staker 1 stakes again - test for 0 value mbr txn, should pass", async () => {
    const { algorand } = fixture;
    const staker = stakingAccounts[1];
    const additionalStake = 5n * 10n ** 6n;
    appClient.algorand.setSignerFromAccount(staker.account!);
    algorand.setSignerFromAccount(staker.account!);
    const stakeTxn = await algorand.createTransaction.assetTransfer({
      assetId: stakeAndRewardAssetId,
...
test("create staker with insufficient tier and stake - expect fail", async () => {
  ...
  console.log("userTier:", userTier.return);
  expect(userTier.return).toBe(0n);
...
test("partial unstake pays pending rewards and updates rewardDebt", async () => {
  const gs0 = await appClient.state.global.getAll();
  const stakedAssetId = gs0.stakedAssetId as bigint;
  const rewardAssetId = gs0.rewardAssetId as bigint;
  ...
  // --- balances before ---
  const stakeBalBeforeReq = await appClient.algorand.client.algod.accountAssetInformation(staker.account!.addr, Number(stakedAssetId)).do();
  ...
  if (!sameAsset) {
    const rewardBalBeforeReq = await appClient.algorand.client.algod
      ...
      .do();
    rewardBalBefore = BigInt(rewardBalBeforeReq.assetHolding?.amount ?? 0n);
  }
  ...
  // --- balances after ---
  const stakeBalAfterReq = await appClient.algorand.client.algod.accountAssetInformation(staker.account!.addr, Number(stakedAssetId)).do();
  ...
  if (!sameAsset) {
    const rewardBalAfterReq = await appClient.algorand.client.algod
      .accountAssetInformation(staker.account!.addr, Number(rewardAssetId))
      .do();
    rewardBalAfter = BigInt(rewardBalAfterReq.assetHolding?.amount ?? 0n);
  }
  ...
  // rewardDebtAfter must checkpoint to floor(stakeAfter * rptBefore / PRECISION)
  // (rpt didn't change during unstake)
  const accruedAfter = mulDivFloor(stakeAfter, rptBefore, PRECISION);
  expect(rewardDebtAfter).toBe(accruedAfter);

  // --- global invariants after ---
  const gs2 = await appClient.state.global.getAll();
  expect(BigInt(gs2.rewardPerToken as bigint)).toBe(rptBefore); // monotonic, no change on unstake
  expect(BigInt(gs2.totalStaked as bigint)).toBe(totalStakedBefore - amount);

  // --- balance assertions ---
  if (sameAsset) {
    // stake asset balance increases by amount + pending (reward pays in same ASA)
    const deltaStake = stakeBalAfter - stakeBalBefore;
    expect(deltaStake).toBe(amount + pending);
  } else {
    // stake asset +amount; reward asset +pending
    const deltaStake = stakeBalAfter - stakeBalBefore;
    const deltaReward = rewardBalAfter - rewardBalBefore;
    expect(deltaStake).toBe(amount);
    expect(deltaReward).toBe(pending);
  }
}, 60_000);
